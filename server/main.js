...
const sqlUpdateUserPassword = "UPDATE user SET password = ? , salt = ? WHERE id = ?"
const sqlFindUserById = "SELECT * FROM USER WHERE id = ?";
...
var findUserById = makeQuery(sqlFindUserById, pool);
...
app.post(API_URI + '/changePassword', auth.required, bodyParser.urlencoded({ extended: true }), bodyParser.json({ limit: "50MB" }), (req, res) => {
    ...
    */
    ...
    findUserById([changePasswordObj.id]).then((result) => {
        if (result.length > 0) {
